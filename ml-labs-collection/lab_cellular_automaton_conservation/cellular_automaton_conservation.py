...
def step(grid):
    N = GRID_SIZE
    new_grid = grid.copy()
    #Фиксированный seed для воспроизводимости (но разный на каждом шаге)
    rng = np.random.default_rng(seed=int(grid.sum() * 1000 + grid[0, 0]))
...
def visualize(saved_states, conservation_history):
    ...
    plt.savefig('tz2_result.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("\nГрафик сохранён: tz2_result.png")
